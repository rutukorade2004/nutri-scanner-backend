...
def predict_health_label(sugar, fat, sodium, protein):
    """Runs prediction on nutrition features."""

    try:
        features = np.array([[float(sugar), float(fat), float(sodium), float(protein)]])
    except ValueError:
        raise ValueError("Inputs must be numeric: sugar, fat, sodium, protein.")

    pred = model.predict(features)

    # If model returned encoded integers, decode
    try:
        decoded = label_encoder.inverse_transform(pred)[0]
        return decoded
    except Exception:
        # Model returns raw string
        if isinstance(pred, (list, tuple, np.ndarray)):
            return pred[0]
        return pred
...
if __name__ == "__main__":
    ...
    try:
        out = predict_health_label(5, 3, 200, 6)
